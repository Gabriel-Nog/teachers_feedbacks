<x-app-layout>
    ...
    <div
        ...
        @foreach ($user->professorAsFeedback as $feedback)
            <div class="w-full">
                <div class="w-fit rounded-md flex items-center font-bold gap-2 text-sm"
                    style="{{ $feedback->like > 0 ? 'color: #0f8;' : 'color: #f03;' }} margin-bottom: 12px;">
                    <iconify-icon icon="mdi:like-outline" width="20" height="20"
                        style="{{ $feedback->like > 0 ? 'color: #0f8' : 'color: #f03; transform: rotate(.5turn);' }}"
                        title="like"></iconify-icon>

                    {{ $feedback->like > 0 ? 'Liked!' : 'Unliked!' }}
                </div>

                <p class="text-gray-400 mb-4 text-lg font-medium">Comentário: {{ $feedback->comment }}</p>
            </div>
        @endforeach
